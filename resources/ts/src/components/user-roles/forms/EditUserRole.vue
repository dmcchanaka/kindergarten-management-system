...
export default defineComponent({
    ...
    setup() {
        ...
        const getUserRoleInfo = async() => {
            if(store.idUserRole){
                userRole.value.splice(0, userRole.value.length, ...store.userRoleList);
                let results = userRole.value.filter((item) => {
                    return item.role_id.toString() == store.idUserRole.toString();
                });
                userRole.value.splice(0,userRole.value.length,...results);
                userRoleForm.value.userRole = userRole?.value[0].description || "";
                userRoleForm.value.userRoleId = userRole?.value[0].role_id || "";

                const idPermissionArray = userRole?.value[0].permissions.map(permission => permission.id);
                userRoleForm.value.selectedPermissions = idPermissionArray;
            } else {
                router.go(-1);
            }
        }
        ...
        const submitUserRole = async() => {
            if(userRoleForm.value.userRole == ''){
                ...
                const inputs = {
                    userRoleId: userRoleForm.value.userRoleId,
                    userRole: userRoleForm.value.userRole,
                    permissions: userRoleForm.value.selectedPermissions,
                };
                await store.updateUserRole(inputs);
                const error = Object.values(store.errors);
                if (error.length === 0) {
                    Swal.fire({
