<template>
  <thead>
    <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal border">
      <th v-if="checkboxEnabled" :style="{ width: '30px' }" class="py-2 px-6 text-left">
        <div
      ...
      <template v-for="(column, i) in header" :key="i">
        <th class="py-2 px-6 text-left"
          :class="{
            'text-left': column.textAlign === 'text-left',
            'text-center': column.textAlign === 'text-center',
            'text-end': column.textAlign === 'text-end',
          }"
