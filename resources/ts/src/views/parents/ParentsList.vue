<template>
    <div
        ...
        <Datatable key="userId" @on-sort="sort" @on-items-select="onItemSelect" :data="tableData" :header="tableHeader"
            ...
            <template v-slot:id="{ row: parent }">
                <a class="mb-1 text-gray-800 fw-bold text-hover-primary fs-6">{{ parent.id }}</a>
            </template>
            <template v-slot:first_name="{ row: parent }">
                <a class="mb-1 text-gray-800 fw-bold text-hover-primary fs-6">{{ parent.first_name }}</a>
            </template>
            <template v-slot:last_name="{ row: parent }">
                <a class="mb-1 text-gray-800 fw-bold text-hover-primary fs-6">{{ parent.last_name }}</a>
            </template>
            <template v-slot:email="{ row: parent }">
                <a class="mb-1 text-gray-800 fw-bold text-hover-primary fs-6">{{ parent.email }}</a>
            </template>
            ...
            <template v-slot:actions="{ row: parent }">
                <a v-if="isPermittedRoute('edit-parent')" @click="editParent(parent.id)"
                ...
                <span v-else>&nbsp;</span>
                <a v-if="isPermittedRoute('delete-parent')" @click="deleteParent(parent.id)"
...
export default defineComponent({
    ...
    setup() {
        ...
        const editParent = (parentId) => {
            store.saveParentId(parentId);
            router.push({ name: "edit-parent" });
        }
        ...
        const deleteParent = async(parentId) => {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let response = await store.removeParent(inputs);
                    const error = Object.values(store.errors);
                    if (error.length === 0) {
                        Swal.fire({
                            title: 'Good job!',
